...
//			$currContent = $UEvent->getContentById($page_id,$content_id);
		...
		$sql="SeLecT c.title, c.content, c.end_date, c.is_highlight FROM tbl_event_content c WHERE c.content_id = 10325 AND c.event_id = 106987 LIMIT 1";
        uDb()->findList($sql);
		
		$sql="SeLecT t.* FROM tbl_event t WHERE t.event_id=106987 LIMIT 1";
        uDb()->findList($sql);
		
		$sql="SeLecT t.* FROM tbl_theme t WHERE t.url_alias='300x300' LIMIT 1";
        uDb()->findList($sql);
		
		$sql="SeLecT c.title, c.content, c.end_date, c.is_highlight FROM tbl_event_content c WHERE c.content_id = 10325 AND c.event_id = 106987 LIMIT 1";
        uDb()->findList($sql);
		
		$sql="SeLecT c.content_id, c.page_id, c.pagetype_id, c.title, c.cover_photo 
			FROM tbl_content c 
			WHERE c.pagetype_id IN(1,4) AND c.published = 1 AND c.publish_date <= NOW() 
			ORDER BY c.publish_date DESC LIMIT 8";
		uDb()->findList($sql);
		
		$sql="SeLecT tbl_spot.spot_name AS title, tbl_spot.spot_id, tbl_spot.avg_rating, tbl_spot.tagline 
			FROM tbl_spot 
			WHERE tbl_spot.published = 1 AND tbl_spot.publish_date <= NOW() 
			ORDER by tbl_spot.hits_past_week DESC 
			LIMIT 0, 8";
        uDb()->findList($sql);
		
		$sql = "SeLecT * from tbl_cat WHERE cat_id=3";
		uDb()->findList($sql);
		
		$sql = "SeLecT SQL_CALC_FOUND_ROWS z.content_id,z.title,z.pagetype_id,z.page_id,z.content,z.author_id, z.area_key,z.cat_id AS main_subcat_id,cat.cat_name AS subcat_name, z.cover,z.update_date,z.ordering FROM ( SELECT d.title,d.content_id,d.pagetype_id, d.page_id, d.content, d.author_id, d3.area_key, d.main_subcat_id AS cat_id,d.cover_photo AS cover,d.update_date,c.ordering FROM tbl_theme_pages c LEFT OUTER JOIN tbl_content d ON (c.pagetype_id = d.pagetype_id AND c.page_id = d.page_id) LEFT OUTER JOIN tbl_content_tags d2 ON d.content_id=d2.content_id LEFT OUTER JOIN tbl_content_location d3 ON d.content_id=d3.content_id LEFT OUTER JOIN tbl_content_cat d4 ON d.content_id=d4.content_id WHERE c.theme_id =9203 AND d.published=1 AND d.publish_date<=now() GROUP BY d.content_id ) z LEFT JOIN tbl_cat cat ON z.cat_id=cat.cat_id GROUP BY z.content_id ORDER BY z.ordering ASC,z.update_date DESC, z.content_id DESC LIMIT 0, 5";	
		uDb()->findList($sql);
