...
class CampaignAnswer{
	...
	const CAMPAIGN_ANS_TABLE='tbl_campaign_sma2016_ans';
	const CAMPAIGN_USER_TABLE='tbl_campaign_sma2016_users';
	...
	function validPost(){
		...
		/* }elseif($post['q3_reason']==''){
			...
			$rt->error='請於以下20個候選單位中選出1個 「最佳數碼社交媒體 - 最佳商場應用程式」';
		}elseif($post['q4_reason']==''){
			$rt->error='你最喜歡商場內哪一個角落?';
		} 
		//personal info
		
		elseif(($post['name'] == '')){
			$rt->error='請輸入你的姓名 (必須與身相符)';
		}
