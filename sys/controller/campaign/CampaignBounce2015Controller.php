...
require_once('bounce2015/campaignAnswer.class.php');
class CampaignBounce2015Controller extends UController
{
		...
		if(isset($_POST['form_action'])){
			...
			if($valid->error){
				...
				if($_POST['form_page']=='5'){
					...
					if(isset($_POST['answer_1'])) uSetCookie('campaign_bounce2015[answer_1]',$_POST['answer_1'],0,"/");
					if(isset($_POST['answer_2'])) uSetCookie('campaign_bounce2015[answer_2]',$_POST['answer_2'],0,"/");
					if(isset($_POST['agree'])) uSetCookie('campaign_bounce2015[agree]',$_POST['agree'],0,"/");
	...
	function actionIndex(){
		...
		$subtitle = null;
		$canonicalUrl = $this->createUrl('/campaign/bounce2015');
		...
		switch($_GET['p']){
			...
			case 5:
			case 6:
				$canonicalUrl = $this->createUrl('/campaign/bounce2015',array('p'=>$_GET['p']));
		...
		$this->pageTitle = getMetaTitle('港生活送你 @BOUNCE  100位 開心到彈起！ 室內彈床樂園試玩 '.$subtitle);
