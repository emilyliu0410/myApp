...
class CampaignAnswer{
	...
	const CAMPAIGN_ANS_TABLE='tbl_campaign_sma2018_ans';
	const CAMPAIGN_USER_TABLE='tbl_campaign_sma2018_users';
	...
	function validPost(){
		...
		//index5
		if(sizeof($post['q1'])!=10){
			$rt->error='請於以下20個候選單位中選出10個 「我最喜愛商場」';
		}elseif(sizeof($post['q2'])!=25){
			$rt->error='請於以下50個候選單位中選出25個 「我最喜愛商場活動」';
		}elseif(sizeof($post['q3'])!=1){
			$rt->error='請於以下10個候選單位中選出1個 「澳門之選10強候選名單」';
		}elseif(sizeof($post['q4'])!=1){
			$rt->error='請於以下20個候選單位中選出1個 「最佳數碼社交媒體 - 最佳商場Facebook專頁」';
		}elseif(sizeof($post['q5'])!=1){
			$rt->error='請於以下25個候選單位中選出1個 「最佳數碼社交媒體 - 最佳商場應用程式」';
		}elseif($post['q6_reason']==''){
			$rt->error='你最希望商場舉辦甚麼活動？原因是什麼？';
		} 
		//personal info
...
//		}
	...
	function store(){
		...
		for($i=0; $i<sizeof($q5); $i++){
			$arr=explode('//',$q5[$i],2);
			$sql.=",";
			$sql.="('".$user_id."','5','".$arr[0]."','".$arr[1]."')";
		}
		$sql.=",('".$user_id."','6','0','".$_POST['q6_reason']."')";  
		...
		$sql="INSERT INTO ".self::CAMPAIGN_USER_TABLE."
			...
			VALUES ('".$_POST['name']."','".$_POST['email']."','".$_POST['hkid']."','".$_POST['contact_phone']."',NOW(),'".$agree1."','".$agree2."')";
		$rid=uDb()->query($sql);
