...
class Index2Controller extends UController
{
//    public $layout = 'index';
	...
	function actionIndex()
	{
		...
		$sql = 'SELECT a.theme_id, a.thumbnail_photo, a.title
				FROM tbl_theme a
				WHERE a.theme_id IN (9201,8894,9117,85,8758,8639)
				ORDER BY FIELD(theme_id,9201,8894,9117,85,8758,8639)
				';
		...
		$this->addCss('css/global/slick/slick.css');
		$this->addCss('css/global/slick/slick-theme.css');
		$this->addCss('css/hk-index.css');
		
		$this->addJs('js/global/slick/slick.js');
	...
	function latestSpot($filter_activity='',$limit=5,$isLogin,$user){
		...
		$today = date('Y-m-d');
		$start_date=date('Y-m-d', strtotime('2017-10-03'));
		$end_date=date('Y-m-d', strtotime('2017-10-03'));
		if (($today >= $start_date) && ($today <= $end_date)){
			$onTopPageIDs = array(204172);
		}
		
		$startdate=date('Y-m-d', strtotime('2016-10-01'));
		$enddate=date('Y-m-d', strtotime('2016-10-01'));
		if (($today >= $startdate) && ($today <= $enddate)){
			$start_date=$startdate;
			$end_date=$enddate;
			$onTopPageIDs = array(109191);
			/* if($today==$start_date){
