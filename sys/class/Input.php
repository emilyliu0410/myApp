...
class Input {
	...
	//html
	static  function html($str,$allowTags='',$allowAttrs='') {

		$str = Html::filterTag($str,$allowTags);
		$str = Html::filterAttr($str,$allowAttrs);
		return $str;
	}
	/* 过滤危险html */
	static  function htmlSafe( $str ) {
		/* 过滤style标签 */
	...
	static  function htmlSafeUtravel( $str ) {
		//remove the iframe tag with fackbook link
		$str = preg_replace("/<iframe\s[^>]*src=(\\\)?[\'\"](http:)?\/\/(www\.)?facebook\.com[^>]+>[^>]+>/i",'',$str);
		//debug($str);
		/* 过滤style标签 */
		return preg_replace_callback(
				/* 过滤style标签内容 */
				...
				preg_replace(
					array(
						...
						'/\<\!\-\-.*?\-\-\>/i',
						/* 删除标签：script、link、object、embed、iframe、frame、frameset */
						'/\<\s*(script|object|embed|link|frame(set)?)[^\>]*\>(.*?\<\s*\/\s*\\1\s*\>)?/i',
						/* 删除事件、javascript协议、css表达式 */
						'/\<[^\>]+((on[a-z]+\s*\=|(javascript|vbscript|behavior)\s*\:[^\;\"\\\']|(import|expression)\s*\()[^\>]*)+\>?/i',
					),
					'',
