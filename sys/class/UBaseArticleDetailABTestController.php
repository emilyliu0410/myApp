...
abstract class UBaseArticleDetailABTestController extends UController
{
	var $UTheme;
	var $UContent;
	var $infinite_scroll;
	...
	private function prepareData($page_id, $alias = null, $is_Infinite = false)
    {
		...
		if($_id>0 && ENABLE_SEO_FRIENDLY){
			...
			$this->redirect301($this->getPageUrlPath().'s',$parameters);
		}
		...
		if(count($exclude_ids_arr)<9){
			...
			if($next_article->page_id){
				$next_article_url = $this->createUrl($this->getPageUrlPath().'s' ,array('id'=>$next_article->page_id)).'?scroll=1&exclude='.$exclude_ids;
				...
				$inline_datas[] = 'data-next-url="'.$this->createUrl($this->getPageUrlPath().'s',array('id'=>$next_article_id,'url_title'=>$next_article->title),false).'"';
		...
		$inline_datas[] = 'data-page-url="'.$this->createUrl($this->getPageUrlPath().'s',array('id'=>$article->id,'url_title'=>$article->title),false).'"';
		if($next_article->page_id){
		}
    ...
    protected function prepareRes()
    {
		...
		$this->addCss('abtest/css/hk-article.css');
