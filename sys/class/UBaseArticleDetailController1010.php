...
abstract class UBaseArticleDetailController extends UController
{
	...
	private function prepareData($page_id, $alias = null, $is_Infinite = false)
    {
		...
		if($this->getPageType()==UTour::PAGETYPE_ID){
			if($article->cover->fb_cover_photo){
				$cover_photo = UTour::getFbCoverPath().'1024x576/'.$article->cover->fb_cover_photo;
				
				// check ratio 16:9
				$cover_photo_file = str_replace(UAPP_HOST.UAPP_BASE_URL.'/','',$cover_photo);
				if(!file_exists($cover_photo_file)){
					$cover_photo = UTour::getFbCoverPath().$article->cover->fb_cover_photo;
				}
			}else{
		...
		$share_button = share_with_bookmark_buttons2(array('title'=>$article->title,'url'=>$shareUrl,'pageId'=>$page_id,'pageType'=>$this->getPageType(),'isLogin'=>$isLogin,'isFollowed'=>$isFollowed));
