...
class UController {
    ...
    public $layoutDir;
    public $vars = array();
    public $pageTitle = CST_SITE_NAME;
    public $metaKeywords = CST_DEFAULT_META_KEYWORDS;
    public $metaDescription = CST_DEFAULT_META_DESCRIPTION;
    public $metaPublisher = CST_SITE_PUBLISHER;
    public $metaAuthor = CST_SITE_AUTHOR;
    public $metaOgType = CST_OG_TYPE;
	public $metaOgTitle = CST_SITE_NAME;
	public $metaOgDescription = CST_DEFAULT_META_DESCRIPTION;
...
//		self::writeFileLog('utapp/search',"$time,$page_type,$page,$order,$region,$country,$city,$suburb,$category,$tag,$keyword,$_device,$_os,$_version,$ip",'search');
   ...
   //				debug($url);
    ...
    function createUrl($url, $query = array(), $full_url = true) {
        if(ENABLE_SEO_FRIENDLY){
            switch($url){
                ...
                case '/theme/index':
                case '/tour/detail':
                case '/tour/location/index':
                case '/tour/theme/index':
                case '/tour/all':
                case '/topic/detail':
                case '/topic/index':
                ...
                case '/tag/index':
                case '/category/index':
				case '/advancedsearch/index':
                case '/index':
                case '/offer/index':
                case '/new/index':
                case '':
                case '/campaign/search2018/activity/detail':
                case '/campaign/search2018/spot/detail':
                case '/campaign/search2018/tour/detail':
                case '/campaign/search2018/topic/detail':
    ...
    function createSeoFriendlyUrl($url, $query = array(), $full_url = true) {
        ...
        if(!$full_url) $rt = UAPP_BASE_URL . $url;
    ...
    /**
     * create the url for location tag
     *
     * @param $location_tag
     * @return string
     * */
    function createLocationUrl($location_tag) {
        if(isset($location_tag->url['location'])){
            return $this->createUrl('/location/index', array('id' =>$location_tag->url['location'],'url_title'=>$location_tag->name));
        }else{
            return $this->createUrl('/search/index',$location_tag->url);
        }
    }


    /**
     * create the url for category tag
     *
     * @param $category_id
     * @param $category_name
     * @return string
     */
    function createCategoryUrl($category_id, $category_name) {
        return $this->createUrl('/category/index', array('id' => $category_id, 'url_title'=>$category_name));
    }
    ...
    function getTopPageviewCategoryList($limit=10){
        $sql = "SELECT a.cat_id, a.cat_name, (a.event_count + a.topic_count + a.tour_count + a.spot_count) totalCount
				FROM tbl_cat a
				WHERE (a.event_count + a.topic_count + a.tour_count + a.spot_count) > 0
				ORDER BY totalCount DESC
				LIMIT 0, $limit
				";
        return uDb()->findList($sql);
    }
    function getTopPageviewTagList($limit=10){
        $sql = "SELECT a.tag_id, a.tag_name, (a.event_count + a.topic_count + a.tour_count + a.spot_count) totalCount
				FROM tbl_tags a
				WHERE (a.event_count + a.topic_count + a.tour_count + a.spot_count) > 0
				ORDER BY totalCount DESC
				LIMIT 0, $limit
